...
if (loggedIn == 0) {
    ...
    function login() {
        ...
        $('.login-menu').load(fullSiteUrl.replace(":80", "") + 'login .login-menu', function (response, status) {
            if (status == "error") {
    ...
    function register() {
        ...
        $('.register-menu').load(fullSiteUrl.replace(":80", "") + 'register .register-menu', function (response, status) {
            if (status == "error") {
                $('#loginRegisterError').modal();
                $('.load-wrapper').hide();
            } else {
